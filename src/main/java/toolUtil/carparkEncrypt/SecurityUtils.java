...
public class SecurityUtils {
    ...
    /**
     * decryptAndVerifyAndURLDecoder 与encryptAndSignAndURLEncoder方法成对使用
     * 解密且验签,将通过URLDecoder解码签名以及加密数据,并做验签操作,
     * 验签时:需要对解密后的明文json数据asii码排序,并通过&连接,然后再进行验签操作
     */
    public static DecryptAndVerifyResBean decryptAndVerifyAndURLDecoder(
            DecryptAndVerifyReqBean decryptAndVerifyReqBean) {
        ...
        if (null == decryptAndVerifyReqBean || !StringUtils.hasText(decryptAndVerifyReqBean.getMd5SaltKey())
                || !StringUtils.hasText(decryptAndVerifyReqBean.getTripleDESKey())
                || !StringUtils.hasText(decryptAndVerifyReqBean.getSign())
    ...
    /**
     * encryptAndSignAndURLEncoder与 decryptAndVerifyAndURLDecoder成对使用
     * 加密并加签,并对加密签名数据分别做URLEncoder转码 将请求map中text为json格式的数据,做加密并做URLEncoder.encode
    ...
    public static EncryptAndSignResBean encryptAndSignAndURLEncoder(EncryptAndSignReqBean encryptAndSignReqBean) {
        ...
        if (null == encryptAndSignReqBean || !StringUtils.hasText(encryptAndSignReqBean.getMd5SaltKey())
                || !StringUtils.hasText(encryptAndSignReqBean.getTripleDESKey())
                || !StringUtils.hasText(encryptAndSignReqBean.getPlain())) {
    ...
    public static String genQueryStringByJsonAsii(String jsonStr) throws Exception {
        TreeMap<String, Object> requestStrTreeMap = JsonUtils.jsonToObject(jsonStr, TreeMap.class);
    ...
    public enum SECURITYERRORMES {
        SIGN_FAIL("签名失败"), VERIFY_FAIL("验签失败"), SIGN_NULL("签名为空"), SALT_NULL("MD5盐值为空"), EMPTY_PARAM("空的参数"),
        CIPHERTEXT_NULL("空的加密信息"), CIPHERKEY_NULL("3des密钥为空"), PLAIN_NULL("明文数据为空"), DECRYPT_FAIL("解密失败"),
        DECRYPT_EMPTY("解密后为空");
